...
const DiscoveryProgressTracker = ({ phases, answers, currentPhaseIndex }) => {
  // Memoize expensive calculations for phase statuses
  const phaseStatuses = React.useMemo(() => {
    return phases.map((phase, phaseIndex) => {
      const phaseQuestionKeys = phase.questions.map(q => q.key);
      const answeredKeysInPhase = phaseQuestionKeys.filter(key => answers[key] !== undefined);
  ...
  const totalQuestionsDisplay = phases.reduce((acc, phase) => acc + phase.questions.length, 0);
