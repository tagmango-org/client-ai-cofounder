...
export const InvokeLLM = async (payload) => {
  try {
    ...
    const systemMessage = `Respond in JSON format. Schema: ${JSON.stringify(response_json_schema, null, 0)}
    
    Rules:
    - Valid JSON only
    - Always populate ai_response_text
    - For courses: Create 4-6 modules with 3-5 chapters each
    - Fill content fields with 200+ words each
    - Include practical examples and steps
    - Structure: Introduction → Fundamentals → Intermediate → Advanced → Applications → Conclusion`;
...
console.log('🔑 API Key configured:', !!OPENAI_API_KEY && OPENAI_API_KEY !== 'your-api-key-here');
